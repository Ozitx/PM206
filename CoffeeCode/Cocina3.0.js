...
// + No tendrá una moneda para un café?
// - Un qué?
// + Un café...
// - Un qué?
// + Un café...

function prepararCafe(nombreProducto) {
    return new Promise(function(resolve, reject) {
        ...
        setTimeout(function() {
            ...
            if (azar < 0.6) {
                resolve ("Tu café está listo!!!" + nombreProducto);
            } else if (azar < 0.8) {
                reject("Error de cocina al preparar: " + nombreProducto);
            } else {
                reject("Faltan ingredientes para: " + nombreProducto);
            }
        }, 2000);
    });
}

async function simularPreparacion(nombreProducto) {
    console.log(`\nPreparando "${nombreProducto}"... espera un momento.`);

    try {
        const resultado = await prepararCafe(nombreProducto);
...
function menuCocina() {
    ...
    console.log("7. Simular preparacion");
    ...
    terminal.question("Opción: ", function(opcion) {
        if (opcion == "1") {
            ...
            listarProductos();
            terminal.question("ID del producto a preparar: ", function(idBuscar) {
                const producto = productos.find(function(p) {return p.id == idBuscar;})
                ...
                simularPreparacion(producto.nombre).then(function() {
                    menuCocina();
                });
            });
        }
